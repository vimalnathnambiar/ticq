...
# Check new sample status
if (newSampleStatus) {
  # Column names of extracted spectral and metadata
  sampleID <- "sampleID"
  project <- "project"
  cohort <- "cohort"
...
if (newSampleStatus && metadataAnonymiser) {
  ...
  saveRDS(passedData, file = paste0(outputDirectoryPath, params$title, "_anonymised.rds"))
}
...
if (newSampleStatus) {
  ...
  if (historicalDataStatus) {
    ...
    if (identical(colnames(historicalData), colnames(passedData))) {
      passedData <- rbind(historicalData, passedData)
    } else {
      message("Unable to append historical data: Non-identical data columns")
    }
    rm(historicalData)
    invisible(gc())
  }
  ...
  if (spectralDataStatus) {
    ...
    if (historicalDataStatus) {
      ...
      if (identical(colnames(historicalData), colnames(passedData)) &&
        ...
        message("Unable to append historical data: Non-identical data columns or spectra array length")
      }
      rm(historicalData)
      invisible(gc())
    }
