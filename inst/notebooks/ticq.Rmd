...
params:
    ...
    outputPath: NULL
    historicalDataPath: "/run/media/vimalnathnambiar/WD/data/historical/test.rds"
    targetFilePath: NULL
...
# Check input path and update new sample status
...
if (is.null(outputPath) || !is.character(outputPath) || outputPath == "") {
  message("Invalid output path: Setting default")
...
## Historical Data
```{r, echo = FALSE, warning = FALSE}
# Check historical data path and update historical data status
...
## Target File
```{r, echo = FALSE, warning = FALSE}
# Check target file and update target file status
targetFile <- ticq::extractTargetFile(
...
## Chromatogram Region
```{r, echo = FALSE, warning = FALSE}
# Check chromatogram region data
...
if (is.null(chromatogramRegion) && !is.null(anpcMethodLibrary) && is.character(anpcMethodLibrary)) {
  message("Attempting to use ANPC method library associated chromatogram region data")
  # ANPC method library chromatogram region data
  ...
  if ((anpcMethodLibrary %in% names(anpcChromatogramRegionData))) {
    ...
    message("Unable to configure chromatogram region data: No matching ANPC method library chromatogram region data available")
  }
  
  # Clean environment
  rm(anpcChromatogramRegionData)
  invisible()
}
...
# Check new sample status
if (newSampleStatus) {
  # Column names of extracted spectral data used for assessment
  sampleID <- "sampleID"
  project <- "project"
  cohort <- "cohort"
  ...
  if (any(pattern %in% sampleTypeList)) {
    ...
    for (i in unique(sub("sLTR |LTR ", "", pattern))) {
      ...
      if (any(tmpPattern %in% sampleTypeList)) {
        ...
        analysisResult_TS <- ticq::displayTimeSeries(
          data = plotData %>%
            dplyr::filter(.data[[sampleType]] %in% tmpPattern),
          x = x,
          y = y,
  ...
  if (any(pattern %in% sampleTypeList)) {
    ...
    for (i in unique(sub("sLTR |LTR ", "", pattern))) {
      ...
      if (any(tmpPattern %in% sampleTypeList)) {
        ...
        analysisResult_VP_massCalibrationRegion <- ticq::displayViolinBox(
          data = plotData %>%
            dplyr::filter(.data[[sampleType]] %in% tmpPattern),
          x = x,
          y = y,
...
if (newSampleStatus && spectralDataStatus && chromatogramRegionStatus) {
  ...
  if (any(pattern %in% sampleTypeList)) {
    ...
    for (i in unique(sub("sLTR |LTR ", "", pattern))) {
      ...
      if (any(tmpPattern %in% sampleTypeList)) {
        # Perform violin plot analysis
        analysisResult_VP_analyteRegion <- ticq::displayViolinBox(
          data = plotData %>%
            dplyr::filter(.data[[sampleType]] %in% tmpPattern),
          x = x,
          y = y,
...
if (newSampleStatus && targetFileStatus && extractedIonStatus && chromatogramRegionStatus && spectralDataStatus && targetMzStatus) {
  ...
  if (any(pattern %in% sampleTypeList)) {
    # Perform time series analysis
    analysisResult_TS_massCalibrationReference <- ticq::displayMultipleTimeSeries(
      data = plotData_filtered %>%
        dplyr::filter(.data[[sampleType]] %in% pattern),
      commonColumn = commonColumn,
      x = x,
...
if (newSampleStatus && targetFileStatus && extractedIonStatus && chromatogramRegionStatus && spectralDataStatus && targetMzStatus) {
  ...
  if (any(pattern %in% sampleTypeList)) {
    # Perform time series analysis
    analysisResult_TS_internalStandard <- ticq::displayMultipleTimeSeries(
      data = plotData_filtered %>%
        dplyr::filter(.data[[sampleType]] %in% pattern),
      commonColumn = commonColumn,
      x = x,
...
if (newSampleStatus && targetFileStatus && extractedIonStatus && chromatogramRegionStatus && spectralDataStatus && targetMzStatus) {
  ...
  if (any(pattern %in% sampleTypeList)) {
    # Perform time series analysis
    analysisResult_TS_endogenousAnalyte <- ticq::displayMultipleTimeSeries(
      data = plotData_filtered %>%
        dplyr::filter(.data[[sampleType]] %in% pattern),
      commonColumn = commonColumn,
      x = x,
...
if (newSampleStatus && targetFileStatus && extractedIonStatus && chromatogramRegionStatus && spectralDataStatus && targetMzStatus) {
  ...
  if (any(pattern %in% sampleTypeList)) {
    # Perform time series analysis
    analysisResult_TS_product <- ticq::displayMultipleTimeSeries(
      data = plotData_filtered %>%
        dplyr::filter(.data[[sampleType]] %in% pattern),
      commonColumn = commonColumn,
      x = x,
...
if (newSampleStatus && targetFileStatus && extractedIonStatus) {
  ...
  plotData <- ticq::countSpectrum(data = massAccuracyData, commonColumn = commonColumn, spectrumCount)
  massAccuracyData <- ticq::calculateMassAccuracy(
    ...
    for (i in 1:length(uniqueMsLevel)) {
      ...
      if (length(targetMZ_filtered) > 0) {
        plotData <- massAccuracyData %>%
          ...
          ticq::calculateAverageMassAccuracy(
            data = .,
            commonColumn = commonColumn,
            spectrumCount = spectrumCount,
...
# Clean environment
  ...
  rm(params, outputPath, chromatogramRegion, roundDecimalPlace, newSampleStatus, historicalDataStatus, targetFileStatus,
