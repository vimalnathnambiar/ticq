...
# Extract target m/z data from target file or in-house ANPC method library
...
if (newStatus && msStatus) {
  ...
  # Clean environment
  rm(labelX, labelY, boundary, xTickToggle, pattern, i, result)
  invisible(gc())
}
```
...
if (newStatus && msStatus) {
  # Set plot variables and labels
  labelY <- "Mass Calibration Region (TIC)"
    ...
    for (i in pattern) {
      ...
      if (i %in% sampleTypeList) {
        ...
        tmpReferenceData <- plotData %>%
          dplyr::filter(.data[[sampleType]] %in% tmpPattern)
        if (nrow(tmpReferenceData) > 1) {
          boundaryValue <- ticq::generateStat(data = tmpReferenceData[[massCalRegion]]) %>%
            dplyr::select(mean, sd, sd2) %>%
            as.list()
        } else {
          boundaryValue <- 0
    ...
    for (i in pattern) {
      ...
      if (i %in% sampleTypeList) {
        ...
        tmpReferenceData <- plotData %>%
          dplyr::filter(.data[[sampleType]] %in% tmpPattern)
        if (nrow(tmpReferenceData) > 1) {
          boundaryValue <- ticq::generateStat(data = tmpReferenceData[[analyteRegion]]) %>%
            dplyr::select(mean, sd, sd2) %>%
            as.list()
        } else {
          boundaryValue <- 0
...
The report highlights the performance of the mass calibration references present within the samples.
...
### Analyte Region
The report highlights the combined performance of the internal standards and endogenous analytes present within the samples.
...
Chromatogram overview (Base Peak Intensity, BPI vs Retention Time, RT) of each sample type along with 3 chromatographic regions of interest outlined: mass calibration, analyte, and wash region. Chromatogram is plotted using full scan MS level 1 spectral data. 
```{r, echo = FALSE, warning = FALSE}
# Check if there are new samples to process
if (newStatus) {
  # Check if MS level 1 spectral data is available
  if (any(passedData[[msLevel]] == 1)) {
    ...
    msStatus <- FALSE
    print("Unable to generate Base Peak Chromatogram: No available MS level 1 spectral data")
  }
} else {
  print("Unable to generate Base Peak Chromatogram: No new samples to analyse")
}
```
...
if (newStatus && msStatus) {
  ...
  # Clean environment
  rm(plotData, title, labelX, labelY, colour, colourLegend, pattern, i)
  invisible(gc())
}
...
Chromatogram overview (Extracted Ion Current, EIC vs Retention Time, RT) of targeted mass calibration references, internal standards, endogenous analytes, and product ions by sample type. Chromatogram is plotted using MS level 1 spectral data for mass calibration references, internal standards, and endogenous analytes; whereas MS level 2 spectral data is used for product ions. 
```{r, echo = FALSE, warning = FALSE}
# Check if there are new samples to process and target m/z data is available
if (newStatus && !is.null(targetMZ)) {
  # Check new sample status by filtering target m/z data based on MS level detected in passed spectral data
...
if (newStatus) {
  ...
  if (any(passedData[[msLevel]] == 1)) {
    ...
    # Extract and append intensity (EIC) of each target m/z stored in spectra into their own respective columns
...
# Check if there are new samples to process and MS level 1 spectral data is available
if (newStatus && msStatus) {
  # Filter and sort unique target m/z values
  targetList <- targetMZ %>%
    dplyr::filter(analyteType == "Mass Calibration"
                  & msLevel == 1)
  targetList <- as.character(sort(unique(targetList$mzValue)))
  
  # Check target status (based on availability of target list)
  if (length(targetList) > 0) {
    targetStatus <- TRUE
    
    # Filter plot data for specific chromatogram region
    plotData_filtered <- ticq::filterChromatogramRegion(data = plotData %>%
                                                          dplyr::select(all_of(c(commonColumn, 
                                                                                 retentionTime,
                                                                                 targetList))),
  ...
  print("Unable to generate Extracted Ion Chromatogram: No available MS level 1 spectral data")
}
```
...
#### LTR Plasma
```{r, echo = FALSE, warning = FALSE}
# If there are new samples to process and MS level 1 spectral data and target list is available
...
if (newStatus && msStatus) {
  # Filter and sort unique target m/z values
  targetList <- targetMZ %>%
    dplyr::filter(analyteType == "Internal Standard"
                  & msLevel == 1)
  targetList <- as.character(sort(unique(targetList$mzValue)))
...
if (newStatus && msStatus) {
  # Filter and sort unique target m/z values
  targetList <- targetMZ %>%
    dplyr::filter(analyteType == "Analyte"
                  & msLevel == 1)
  targetList <- as.character(sort(unique(targetList$mzValue)))
...
if (newStatus && msStatus) {
  # Clean environment
  rm(msStatus, plotData, colour, colourLegend, targetList, targetStatus)
}
```
...
# If there are new samples to process
if (newStatus) {
  # Check if MS level 2 spectral data is available
  if (any(passedData[[msLevel]] == 2)) {
    msStatus <- TRUE
    
    # Filter MS level 2 spectral data for plotting
    plotData <- passedData %>% 
    ...
    if (file.exists(historicalDataPath)) {
      ...
      # Filter historical data for MS level 2 spectral data
      historicalData <- historicalData %>%
        dplyr::filter(.data[[msLevel]] == 2)
...
## Product Ions {.tabset}
```{r, echo = FALSE, warning = FALSE}
# Check if there are new samples to process and MS level 2 spectral data is available
if (newStatus && msStatus) {
  # Filter and sort unique target m/z values
  targetList <- targetMZ %>%
    dplyr::filter(analyteType == "Product"
                  & msLevel == 2)
  targetList <- as.character(sort(unique(targetList$mzValue)))
  ...
  print("Unable to generate Extracted Ion Chromatogram: No available MS level 2 spectral data")
}
```
...
#### LTR Plasma
```{r, echo = FALSE, warning = FALSE}
# If there are new samples to process and MS level 2 spectral data and target list is available
if (newStatus && msStatus && targetStatus) {
  # Set patterns to check
  pattern <- c("sLTR Plasma", "LTR Plasma")
...
if (newStatus) {
  # Clean environment
  rm(commonColumn, caption, labelX, labelY, nPlotCol, nPlotRow)
}
```
