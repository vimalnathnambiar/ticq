...
if (newStatus && msStatus) {
  ...
  # Clean environment
  rm(x, y, title, labelX, labelY, pattern, i)
}
```
    ...
    for (i in pattern) {
      ...
      if (i %in% sampleTypeList) {
        ...
        if (nrow(tmpReferenceData) > 1) {
          boundaryValue <- ticq::generateStat(data = tmpReferenceData[[y]]) %>%
...
if (newStatus && msStatus) {
  ...
  # Clean environment
  rm(x, y, labelX, labelY, boundary, xTickToggle, pattern, i, result)
  invisible(gc())
}
```
...
if (newStatus && msStatus) {
  ...
  # Clean environment
  rm(plotData, x, y, title, labelX, labelY, colour, colourLegend, pattern, i)
  invisible(gc())
}
...
if (newStatus) {
  # Clean environment
  rm(commonColumn, x, caption, labelX, labelY, nPlotCol, nPlotRow)
}
```
...
if (newStatus && msStatus) {
  ...
  if (length(targetList) > 0) {
    ...
    # Data frame to store analysis result
    analysisResult_TS_massCalibration <- data.frame()
  } else {
    targetStatus <- FALSE
    print("Unable to generate Extracted Ion Chromatogram: No available target m/z data")
  }
...
# If there is new samples to process and MS1 data and target m/z data is available
if (newStatus && msStatus && targetStatus) {
  # Set patterns to check
  pattern <- c("sLTR Plasma", "LTR Plasma")
  ...
  if (any(pattern %in% sampleTypeList)) {
    ...
    analysisResult_TS_massCalibration <- rbind(analysisResult_TS_massCalibration, result) 
  } else { 
    print("No available data") 
  }
}
