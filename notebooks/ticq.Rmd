...
chromatogramRegion <- ticq::configureChromatogramRegion(massCalStart = 0, massCalEnd = 0.3, washStart = 5)
...
```{r, echo = FALSE, warning = FALSE, include = FALSE}
# Historical data path (In-house ANPC method specific)
...
# Check new sample status
if (newStatus) {
  # Column names of extracted spectral data used in workflow
  sampleID <- "sampleID"
  project <- "project"
  cohort <- "cohort"
...
# Check new sample status and spectral data status
if (newStatus && msStatus) {
  # Splice chromatogram regions for analysis and plotting
...
```{r, echo = FALSE, warning = FALSE}
# Check new sample status and if target m/z data is available
if (newStatus && !is.null(targetMZ)) {
  # Update new sample status
  if (length(unique(passedData[[arrayLength]])) != 1) {
    newStatus <- FALSE
    print("Unable to display Extracted Ion Chromatogram: Samples do not share the same spectra array length")
  } else if (length(unique(targetMZ$mzValue)) != unique(passedData[[arrayLength]])) {
    newStatus <- FALSE
    print("Unable to display Extracted Ion Chromatogram: Length of unique target m/z does not match length of available spectra array")
  }
} else if (!newStatus) {
  print("Unable to display Extracted Ion Chromatogram: No new samples to analyse")
...
# Check new sample status
if (newStatus) {
  # Check if spectral data and associated target m/z is available
  if (any(passedData[[msLevel]] == 1) && any(targetMZ$msLevel == 1)) {
    # Update spectral data status
    msStatus <- TRUE
...
if (newStatus && msStatus) {
  ...
  # Check if target m/z is available
  ...
  print("Unable to display Extracted Ion Chromatogram: No available MS level 1 spectral (and/or associated target m/z) data")
}
```
...
if (newStatus) {
  # Check if spectral data and associated target m/z is available
  if (any(passedData[[msLevel]] == 2) && any(targetMZ$msLevel == 2)) {
    # Update spectral data status
    msStatus <- TRUE
...
if (newStatus && msStatus) {
  ...
  print("Unable to display Extracted Ion Chromatogram: No available MS level 2 spectral (and/or associated target m/z) data")
}
```
...
if (newStatus && msStatus) {
  ...
  print("Unable to perform Time Series Analysis: No available MS level 1 spectral (and/or associated target m/z) data")
}
```
...
if (newStatus && msStatus) {
  ...
  print("Unable to perform Time Series Analysis: No available MS level 2 spectral (and/or associated target m/z) data")
}
```
...
Mass accuracy analysis provides a trendline (mass accuracy shift) and assessment of instrument performance in detecting a target m/z based on the average mass error (in ppm). A report is generated highlighting the target m/z that have a significant 1st and 2nd standard deviation from the average mean performance.
```{r, echo = FALSE, warning = FALSE}
# Check new sample status
if (newStatus) {
  # Common data columns
  ...
  # Target list
  targetList <- sort(unique(targetMZ$mzValue))
  ...
  # Extract and calculate average mass error of each unique target m/z
  plotData <- ticq::extractMassError(data = plotData,
                                     mzArray = mzArray,
                                     targetList = targetList)
  
  plotData <- ticq::calculateAverageMassError(data = plotData,
                                              commonColumn = commonColumn,
                                              spectrumCount = spectrumCount,
                                              startIDX = which(colnames(plotData) == as.character(targetList[1])),
                                              roundDecimal = roundDecimal)
  ...
  # Plot time series
  result <- ticq::plotMultipleTimeSeries(data = plotData,
                                         commonColumn = commonColumn,
                                         x = x,
                                         startIDX = startIDX,
                                         colour = colour,
  ...
  print("Unable to perform Mass Accuracy Analysis: No new samples to analyse")
}
```
