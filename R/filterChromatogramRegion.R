...
filterChromatogramRegion <- function(data, regionOfInterest = NULL, filterBy) {
  # Check if region of interest is NULL
  if (is.null(regionOfInterest)) {
    regionOfInterest <- list(start = 0, end = max(data[[filterBy]]))
  } else {
  ...
  # Filter data
  regionData <- data %>%
    dplyr::filter(.data[[filterBy]] > regionOfInterest$start & .data[[filterBy]] <= regionOfInterest$end)
