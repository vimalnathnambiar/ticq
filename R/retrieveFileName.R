#' Retrieve File Name
#'
#' Filter and retrieve file names of a specific extension type stored within an input path.
#'
#' @export
...
#' @returns File names matching the file extension defined: character vector
retrieveFileName <- function(input, fileExtension = NULL) {
  ...
  if (dir.exists(input) || file.exists(input)) {
    ...
    # Check input type (directory or file path) for file name retrieval
    if (fileInfo$isdir) {
