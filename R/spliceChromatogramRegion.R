...
#'
#' Splice and sum values for each chromatogram regions of interest.
#'
#' @export
...
spliceChromatogramRegion <- function(data,
  ...
  regionData <- ticq::sumDataColumn(data = data,
                                    commonColumn = commonColumn,
                                    sumColumn = sumColumn)
  ...
  if (!is.null(chromatogramRegion)) {
    # Prewash region
    regionData <- ticq::filterChromatogramRegion(data = data,
                                                 regionOfInterest = chromatogramRegion$prewash,
                                                 retentionTime = retentionTime)
    regionData <- ticq::sumDataColumn(data = regionData,
                                      commonColumn = commonColumn,
                                      sumColumn = sumColumn)
    ...
    regionData <- ticq::sumDataColumn(data = regionData,
                                      commonColumn = commonColumn,
                                      sumColumn = sumColumn)
    splicedData <- cbind(splicedData, massCalRegion = regionData$sum)
